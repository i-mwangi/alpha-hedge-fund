...
show_help() {
  echo "Alpha hedge fund Docker Runner"
  echo ""
...
if [ -n "$USE_OLLAMA" ]; then
  ...
  # Build the Alpha hedge fund image if needed
  if [[ "$(docker images -q ai-hedge-fund 2> /dev/null)" == "" ]]; then
    echo "Building Alpha hedge fund image..."
    docker build -t ai-hedge-fund .
  fi
  ...
  echo "Running Alpha hedge fund with Ollama using Docker Compose..."
